...
def generate_combined_prompts_one(db_path, question, knowledge=None):
    ...
    return schema_prompt,comment_prompt,cot_prompt
...
def collect_response_from_gpt(db_path_list, question_list, api_key, engine, knowledge_list=None):
    ...
    for i, question in tqdm(enumerate(question_list)):
        ...
        if knowledge_list:
            schema_prompt,comment_prompt = generate_combined_prompts_one(db_path=db_path_list[i], question=question, knowledge=knowledge_list[i])
        else:
...
def get_final_prompt(db_path_list, question_list,db_id_list, knowledge_list=None):
    prompt_list = []
    for i, question in tqdm(enumerate(question_list)):
        print('--------------------- processing {}th question ---------------------'.format(i))
        print('the question is: {}'.format(question))
        ...
        if knowledge_list:
            schema_prompt, comment_prompt, cot_prompt = generate_combined_prompts_one(
        ...
        # 将 cur_prompt 作为字典的一项添加到列表中，增加 db_path 键
        prompt_list.append({
            "id": i,
            "db_id": db_id_list[i],  # 添加数据库名
            "question": question,
            "schema": schema_prompt,
            "external knowledge": comment_prompt,
            "cot": cot_prompt
        })

    # 将 prompt_list 转换为 JSON 格式的字符串
    prompt_json = json.dumps(prompt_list, ensure_ascii=False, indent=4)
...
def decouple_question_schema(datasets, db_root_path):
    ...
    for i, data in enumerate(datasets):
        ...
        db_id.append(data['db_id'])
        knowledge_list.append(data['evidence'])

    return question_list, db_path_list,db_id, knowledge_list
...
if __name__ == '__main__':
    ...
    # eval_data = json.load(open('F:/Mercury-SQL/Datasets/BIRD_dev/dev.json', 'r'))
    eval_data = json.load(open('F:/train_bird/train.json', 'r'))

    # question_list, db_path_list, knowledge_list = decouple_question_schema(datasets=eval_data, db_root_path=args.db_root_path)
    # question_list, db_path_list, db_id,knowledge_list = decouple_question_schema(datasets=eval_data, db_root_path="F:/Mercury-SQL/Datasets/BIRD_dev/dev_databases/")# 路径最后要加/
    question_list, db_path_list, db_id,knowledge_list = decouple_question_schema(datasets=eval_data, db_root_path="F:/train_bird/train_databases/")# 路径最后要加/
    assert len(question_list) == len(db_path_list) == len(knowledge_list)
    # if args.use_knowledge == 'True':
    prompt_json = get_final_prompt(db_path_list=db_path_list, question_list=question_list, knowledge_list=knowledge_list,db_id_list=db_id)
    # 保存为 JSON 文件
    with open('output_prompts.json', 'w', encoding='utf-8') as f:
        f.write(prompt_json)
    # else:
